...
所以mutex是可重入的，而semaphore是不可重入的，所以一个线程如果对一个二值信号量进行递归持有会导致当前线程挂起。可以看看rt_sem_take()中的实现：
...
当一个线程获取一个二值信号量时，value会减少到0，当该线程再次获取该信号量时，由于vaule值小于0时，会将当前线程挂起，并将其插入到等待队列中，由于该信号量不会被释放，所以该线程会一直执行，直到定时时间到
由于该线程为挂起状态，故其无法加入就绪队列中，所以该线程将无法得到运行。
