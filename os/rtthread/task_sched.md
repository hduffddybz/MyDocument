...
## 概述
 ...
 实时系统与一般的分时系统的区别在于实时系统在接收一个任务到完成任务的这段抖动时间要尽可能的短。实时系统分为硬实时系统和软实时系统，对于每个任务来说，硬实时系统必须在其截至时间内完成所有任务，而软实时系统只需在平均情况下在其截至时间内完成任务[1]。在汽车、工业、航空等领域，一些任务超过其执行截至时间将产生灾难性的影响（例如汽车中安全气囊弹出的响应），因而实时操作系统有着广泛的应用。著名的实时操作系统包括了FreeRTOS，VxWorks，RT-Linux等。
...
## RT-Thread调度算法设计实现
 ...
 RT-Thread存在多个优先级线程，同时支持相同优先级的线程，对于不同优先级的线程采用可抢占式调度的形式，而对于同一优先级的不同线程采用时间片轮转进行调度。
...
其中的init_priority指的是初始化线程时设定的任务优先级，在运行过程中系统不会动态的调整优先级的大小，但可通过线程控制函数手动更改线程优先级。在实时系统中任务执行的确定性是一个重要因素，动态优先级算法会增加代码实现复杂性，会为系统引入更多的不确定性。
...
算法1：语句1定义了线程控制块数组，其线程最大优先级为256，一般的我们会想到如下的调度算法：
...
```
显而易见的是该算法的时间复杂度为O(N)(N为最大线程优先级)，这在实时系统中是无法接受的。
...
```
这个表只包括了8个优先级的情况，可想而知当优先级增长到256时这个表将存储2**32=4G的空间，嵌入式芯片将无法存放。所以仅仅依靠位图表的做法是不可行的。
